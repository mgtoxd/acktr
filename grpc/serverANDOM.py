...
# 节点数量*文件数量*延迟
class FormatData(landlord2Cloud_pb2_grpc.ACKTRServicer):
...
def serve(i, b: BaseChoose):
    # 定义服务器并设置最大连接数,concurrent.futures是一个并发库，类似于线程池的概念
    grpcServer = grpc.server(futures.ThreadPoolExecutor(max_workers=4))  # 创建一个服务器
    landlord2Cloud_pb2_grpc.add_ACKTRServicer_to_server(FormatData(b), grpcServer)  # 在服务器中添加派生的接口服务（自己实现了处理函数）
    print(str(5000 + i))
...
if __name__ == '__main__':
    ...
    print(sizeMap)
    for i in range(3, 15):
        b = BaseChoose(sizeMap=sizeMap)
        t = threading.Thread(target=serve, args=(i, b))
        t.start()
    while True:
        time.sleep(100)
